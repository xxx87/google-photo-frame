...
/* Настройте кеш для мультимедийных элементов, срок действия которого истекает через 55 минут.
Это кэширует baseUrls для элементов мультимедиа, которые были выбраны пользователем для фоторамки. Они используются для отображения фотографий в виде эскизов и в рамке. BaseUrls отправляются во внешний интерфейс и отображаются оттуда. BaseUrls временно кэшируются, чтобы приложение работало быстро и быстро. Обратите внимание, что эти данные должны храниться только в течение короткого периода времени и что доступ к URL-адресам истекает через 60 минут. См. «Передовой опыт» и «политику допустимого использования» в документации для разработчиков. */
...
/* Временно кешируйте список альбомов, принадлежащих пользователю. Это кеширует имя и базовый URL-адрес изображения обложки. Это гарантирует, что приложение будет реагировать, когда пользователь выбирает альбом. Для загрузки полного списка альбомов, принадлежащих пользователю, может потребоваться несколько запросов. Это временное кэширование позволяет пользователю вернуться к экрану выбора альбома, не дожидаясь выполнения запросов каждый раз. Обратите внимание, что эти данные только временно кэшируются в соответствии с «лучшими практиками» в документации для разработчиков. Здесь он истекает через 10 минут. */
...
/* Для каждого пользователя приложение сохраняет последние параметры поиска или альбом, который они загрузили в фоторамку. При следующем входе в систему (или по истечении срока хранения кешированных данных) этот поиск будет выполнен повторно. Это сохраняет данные свежими. Вместо сохранения параметров поиска мы могли бы также сохранить список идентификаторов элементов мультимедиа и обновлять их, но повторная отправка поискового запроса гарантирует, что фоторамка отображает все новые изображения, соответствующие критериям поиска (или добавленные в альбом. ). */
const storage = persist.create({ dir: "persist-storage/" });
storage.init();

// Настройте аутентификацию OAuth 2.0 через библиотеку Passport.js.
const passport = require("passport");
const auth = require("./auth");
auth(passport);

/* Настройте промежуточное программное обеспечение сеанса для обработки пользовательских сеансов.
ПРИМЕЧАНИЕ. Для подписи файла cookie используется секрет. Он используется только для этого примера приложения и должен быть изменен. */
...
// Включите расширенное ведение журнала, если установлена переменная среды DEBUG
...
// Настройте статические маршруты для размещенных библиотек.
...
// Анализировать данные запроса приложения / json.
...
/* Промежуточное ПО, которое добавляет пользователя этого сеанса в качестве локальной переменной, чтобы он мог отображаться на всех страницах при входе в систему. */
...
/* Запрос GET для выхода пользователя из системы.
Удалите текущий сеанс и перенаправьте обратно на экран входа в систему. */
...
app.post("/loadFromSearch", async (req, res) => {
  ...
  if (req.body.includedCategories) {
    ...
    filters.contentFilter.includedContentCategories = [req.body.includedCategories];
  }

  if (req.body.excludedCategories) {
    // Excluded categories are set in the form. Add them to the filter.
    filters.contentFilter.excludedContentCategories = [req.body.excludedCategories];
  }
