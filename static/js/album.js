  ...
  $.ajax({
    ...
    success: (data) => {
      ...
      $.each(data.albums, (i, item) => {
        ...
        const primaryContentImage = $("<img />").attr("src", thumbnailUrl).attr("alt", item.title).addClass("mdl-list__item-avatar");
        ...
        const primaryContentSubTitle = $("<div />").text(`(${item.mediaItemsCount} items)`).addClass("mdl-list__item-sub-title");
        primaryContentRoot.append(primaryContentSubTitle);

        // Secondary content consists of two links with buttons.
        const secondaryContentRoot = $("<div />").addClass("mdl-list__item-secondary-action");
        materialDesignLiteList.append(secondaryContentRoot);

        // The 'add to photo frame' link.
        const linkToAddToPhotoFrame = $("<a />").addClass("album-title").attr("data-id", item.id).attr("data-title", item.title);
        secondaryContentRoot.append(linkToAddToPhotoFrame);

        // The button for the 'add to photo frame' link.
        const addToPhotoFrameButton = $("<button />").addClass("mdl-button mdl-js-button mdl-button--raised mdl-button--accent").text("Add to frame");
        linkToAddToPhotoFrame.append(addToPhotoFrameButton);

        // The 'open in Google Photos' link.
        const linkToGooglePhotos = $("<a />").attr("target", "_blank").attr("href", item.productUrl);
        secondaryContentRoot.append(linkToGooglePhotos);

        // The button for the 'open in Google Photos' link.
        const googlePhotosButton = $("<button />").addClass("gp-button raised").text("Open in Google Photos");
