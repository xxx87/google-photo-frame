...
function showPreview(source, mediaItems) {
  ...
  $.each(mediaItems, (i, item) => {
    ...
    const model = item.mediaMetadata.photo.cameraModel ? `#Shot on ${item.mediaMetadata.photo.cameraModel}` : "";
    ...
    const linkToFullImage = $("<a />").attr("href", fullUrl).attr("data-fancybox", "gallery").attr("data-width", item.mediaMetadata.width).attr("data-height", item.mediaMetadata.height);
    // Add the thumbnail image to the link to the full image for fancybox.
    const thumbnailImage = $("<img />").attr("src", thumbnailUrl).attr("alt", captionText).addClass("img-fluid rounded thumbnail");
    ...
    const linkToGooglePhotos = $("<a />").attr("href", item.productUrl).text("[Click to open in Google Photos]");
    ...
    // container.
    console.log("linkToFullImage: ", linkToFullImage);
...
$(document).ready(() => {
  ...
  // image.
  $("#startSlideshow").on("click", (e) => $("#images-container a").first().click());
